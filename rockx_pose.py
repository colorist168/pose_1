...
if __name__ == '__main__':
    ...
    while True:
        ...
        for result in results:
            ...
            #test计时-5s
            if start==1 and test==1 and pose==0:
                end_time=time.time()
                during=end_time-begin_time
                print(during)
                if during>=5:
                    print('Test-Done! Congrulations!')
                    start=0
                    se=0
                    test=0
            #test Warning 设置
            elif start==1 and test==1 and pose!=0:
                warning_time=warning_time+1
                print('Nearly Done! Come on!')
                if warning_time==1:
                    warning_start=time.time()
                else:
                    warning_end=time.time()
                    warning_during=warning_end-warning_start
                    if warning_during>=5:
                        print('Fail! Please Restart!')
                        warning_time=0
                        start=0
                        se=0
                        test=0
